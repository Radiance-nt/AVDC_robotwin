...
class GoalGaussianDiffusion(nn.Module):
    ...
    def model_predictions(self, x, t, x_cond, task_embed, clip_x_start=False, rederive_pred_noise=False, guidance_weight=0):
        ...
        if self.objective == 'pred_noise':
            pred_noise = model_output if guidance_weight == 0 else (1 + guidance_weight)*model_output - guidance_weight*uncond_model_output
            ...
            while self.step < self.train_num_steps:
                ...
                if accelerator.is_main_process:
                    ...
                    if self.step != 0 and self.step % self.save_and_sample_every == 0:
                        ...
                        for i in range(gt_xs.shape[0]):
                            for j in range(n_rows):
                                gt_frame = gt_xs[i, j].to(device)
                                pred_frame = all_xs[i, j].to(device)
